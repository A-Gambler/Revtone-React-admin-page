...
const TheHeader = () => {
  ...
  useEffect(() => {
    auth.onAuthStateChanged((user) => {
      if (user) {
        database.ref('business/' + user.uid).get().then((snapshot) => {
          if (snapshot.exists) {
  ...
  const saveBusiness = async () => {
    ...
    var updates = {}
    updates['business/businessContact'] = businessContact;
    updates['business/businessEmail'] = businessEmail;
    updates['business/businessWebSite'] = businessWebSite;
    if (imageLink) {
  ...
  const onAdminHeaderSearch = (value) => {
    ...
    EventEmitter.dispatch('admin-header-search', adminHeaderSearch)
